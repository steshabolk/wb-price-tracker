...
- **tests** : JUnit5, integration using testcontainers

#### :small_blue_diamond: **Parser using Java with Spring Boot & Kafka:**

- the parser updates data from Wildberries on a **schedule**: product prices and product details
- data from the parser is sent to the api using **Kafka**
...
- **postgres**
  ...
  - liquibase [changelog](./wb-price-tracker-api/src/main/resources/db/changelog/) files has the necessary changesets for the database
...
> the database will be initialized with test [data](./wb-price-tracker-api/src/main/resources/db/changelog/) for the example. Password used to log in to the account is `userPassword0`
