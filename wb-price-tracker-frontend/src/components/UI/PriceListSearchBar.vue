...
import { dd_mm_yyyy, yyyy_mm_dd, date2LocalDateTime, deepClone } from '@/services/helper'
	...
	methods: {
		...
		handleForm() {
			if (!this.isRequesting && this.isSearchingParamsValid) {
				...
				params.dateFrom = params.dateFrom ? this.date2LocalDateTime(params.dateFrom, 'from') : null
				params.dateTo = params.dateTo ? this.date2LocalDateTime(params.dateTo, 'to') : null
