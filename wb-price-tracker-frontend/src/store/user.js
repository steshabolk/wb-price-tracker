...
import { UNKNOWN_ERROR, PRODUCT_ALREADY_ADDED, CHECK_INVALID_CODE_RESPONSE, INVALID_CODE } from '@/services/errorMessages'
	...
	actions: {
		...
		async addProductToAccount({ commit, dispatch }, code) {
			...
			await instance
				...
				.catch(error => {
					dispatch('request/setIsRequesting', false, { root: true })
					if (error.response && error.response.data && error.response.data.code && error.response.data.code === PRODUCT_ALREADY_ADDED) {
						dispatch('request/setErrMsg', error.response.data.message, { root: true })
					} else if (error.response && error.response.data && error.response.data.code && CHECK_INVALID_CODE_RESPONSE(error.response.data.code)) {
						dispatch('request/setErrMsg', INVALID_CODE(code.code), { root: true })
					} else {
